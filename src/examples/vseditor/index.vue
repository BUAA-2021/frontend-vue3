...
export default {
  ...
  methods: {
    ...
    addControl({ components, parentId }) {
      ...
      // 默认选中最后一个
      let { component } = findComponentPathById(
        controls,
    ...
    duplicateComponent() {
      ...
      if (pathes.length > 0) {
        ...
        controls.splice(
          selectedIndex + 1,
          0,
          newComponent(this.controls[selectedIndex])
        );
      }
      this.setControls(controls);
  ...
  provide() {
    return {
      ...
      [EVENT_COMPONENT_TRANSFORM]: this.handleTransform,
      [EVENT_COMPONENT_UNSELECT]: this.handleUnselect,
      [EVENT_COMPONENT_DUPLICATE]: this.duplicateComponent,
      [EVENT_COMPONENT_DELETE]: this.deleteComponent,
